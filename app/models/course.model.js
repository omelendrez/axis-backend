...
Course.getAll = (search, result) => {
  ...
  const query = `SELECT c.id, c.code, c.name, c.front_id, c.back_id, c.duration, c.validity, c.cert_id_card, c.opito_reg_code, ct.name cert_type_name FROM course c INNER JOIN certificate_type ct ON c.cert_type = ct.id ${filter} ORDER BY code;`
  console.log(query)
