...
Trainee.findById = (id, result) => {
  sql.query(`SELECT * FROM trainee WHERE id = ${id}`, (err, res) => {
    if (err) {
      log.error("error: ", err)
      result(err, null)
      return
...
Trainee.updateById = (id, trainee, result) => {
  sql.query(
    ...
    [trainee.type, trainee.badge, trainee.last_name, trainee.first_name, trainee.sex, trainee.state, trainee.nationality, trainee.birth_date, trainee.company, trainee.status, id],
    (err, res) => {
      if (err) {
        log.error("error: ", err)
        result(null, err)
      ...
      result(null, { id: id, ...toWeb(trainee) })
    }
  )
}

Trainee.remove = (id, result) => {
  sql.query("SELECT COUNT(1) records FROM tracking WHERE trainee = ?", id, (err, res) => {
    if (err) {
      log.error("error: ", err)
      result(null, err)
      return
