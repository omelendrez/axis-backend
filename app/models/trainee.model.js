...
Trainee.getAll = (pagination, result) => {
  ...
  const queryData = `SELECT t.id, t.type, t.badge, CONCAT(t.last_name,', ', t.first_name) full_name, c.name company_name FROM trainee t INNER JOIN company c ON t.company=c.id ${filter} ORDER BY id DESC ${limits};`
