...
User.findByIdView = (id, result) => {
  const query =
    'SELECT u.id, u.name, u.full_name, u.email, json_arrayagg(json_object(r.id, r.name)) roles, CASE u.status WHEN 1 THEN "Active" ELSE "Inactive" END status FROM user u inner join user_role ur on ur.user = u.id inner join role r on r.id = ur.role WHERE u.id=?'
